...
class Blog(models.Model):
    ...
    image = models.FileField(default = 'temp.jpg', verbose_name='Путь к картинке') #добавление в модель записи блога поле для картинки

    # Методы класса:

    def get_absolute_url(self): #метод возвращает строку URL-адресом записи
    ...
    #Метаданные - вложенный класс, который задает допольнительные параметры модели:
    class Meta:
...
class Comment(models.Model):
    ...
    def __str__(self): # метод возвращает название, используеомого для предствавления отдельных записей в административном разделе
